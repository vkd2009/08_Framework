...
const sendMessage = () => {
  ...
  const chattingObj = {
    "targetNo" : selectTargetNo,     // 메시지 받을 대상의 회원 번호(웹소켓)
    "messageContent" : msg,          // 전달할 메시지 내용
    "chattingNo" : selectChattingNo  // 채팅방 번호 (DB에 저장)
  }
	...
	if(query.length > 0){
		...
		.then(list => {
			...
			for(let member of list){
				...
				const span = document.createElement("span");
				span.innerHTML = `${nickname} ${email}`.replace(query, `<mark>${query}</mark>`); // 입력한 값 노란 색 으로 보이게
