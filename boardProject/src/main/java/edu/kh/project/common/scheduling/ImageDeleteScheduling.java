...
@Component
@Slf4j
@PropertySource("classpath:config.properties") // 파일 가져오기 이유는 경로를 그냥 가져다 쓰면 유지보수에 좋지 않아 사용 권장 x
public class ImageDeleteScheduling {
	
	@Autowired // 서비스 bean 의존성 주입(DI);  -- 외부로 부터 남의 것을 가져오는 것
	...
	public void imageDelete() { 
		...
		for(File server: serverImageList) {
			...
			if(  dbImageList.contains(server.getName())) {
				...
				// 9. DB에 없는 서버 이미지 삭제
				log.info("{} 삭제", server.getName()); // servet.getName()의 값이 {} 안으로 들어간다
				server.delete();
			}
